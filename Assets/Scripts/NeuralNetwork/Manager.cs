...
public class Manager : MonoBehaviour
{
    ...
    public void InitNetworks()
    {
        ...
        {
            NeuralNetwork net = new NeuralNetwork(layers, "Assets/ModelSave.txt");
            net.Load();//on start load the network save
            networks.Add(net);
        }
    }
    ...
    public void SortNetworks()
    {
        ...
        networks.Sort();
        networks[populationSize - 1].Save();//saves networks weights and biases to file, to preserve network performance
        for(int i = 0; i < populationSize / 2; i++)
        {
            networks[i] = networks[i + populationSize / 2].Copy(new NeuralNetwork(layers, "Assets/ModelSave.txt"));
